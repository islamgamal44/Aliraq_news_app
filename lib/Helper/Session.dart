...
Future<Locale?> getLocale() async {
  String? languageCode = await getPrefrence(LANGUAGE_CODE);
...
Locale _locale(String languageCode) {
  Locale? loc;
  for (int i = 0; i < lanLocaleList.length; i++) {
    if (lanLocaleList[i].languageCode == languageCode) {
      loc = lanLocaleList[i];
    }
  }

  return loc ?? DEFAULT_LANG_NAME! */ /*Locale("en", "US")*/ /*;
}*/
...
Future<void> createDynamicLink(BuildContext context, String id, int index,
    String title, bool isVideoId, bool isBreakingNews, String image) async {
  ...
  final response1 = await get(Uri.parse(image));
  ...
  //Share.shareXFiles(shortUrl.toString(), subject: str);
}

//translate string based on language code
String? getTranslated(BuildContext context, String key) {
  return DemoLocalization.translate(key) ?? key;
}
...
Future<void> clearUserSession() async {
  ...
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? lang_id = await getPrefrence(LANGUAGE_ID);
  String? langCode = await getPrefrence(LANGUAGE_CODE);
  String? langCodeData = await getPrefrence(langCode!);
  ...
  setPrefrence(LANGUAGE_CODE, langCode);
  setPrefrence(LANGUAGE_ID, lang_id!); //for Intro Screens
  setPrefrence(langCode, langCodeData!); //for Intro Screens
}
...
String getToken() {
  final claimSet = new JwtClaim(
    ...
    issuer: 'NewsAPP',
    expiry: DateTime.now().add(Duration(days: 30 /*minutes: 10*/)),
