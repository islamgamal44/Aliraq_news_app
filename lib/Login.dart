...
class LoginState extends State<Login> with TickerProviderStateMixin {
  ...
  Future<void> getPolicyPages() async {
    _isNetworkAvail = await isNetworkAvailable();

    if (_isNetworkAvail) {
      var param = {ACCESS_KEY: access_key, LANGUAGE_ID: CUR_LANGUAGE_ID};
      Response response = await post(Uri.parse(getPolicyPagesApi),
      ...
      if (error == "false") {
        setState(() {
          privacyTitle = getdata[PRIVACYPOLICY][0][TITLE];
          privacyDesc = getdata[PRIVACYPOLICY][0][PAGE_CONTENT];
          termsTitle = getdata[TERMSPOLICY][0][TITLE];
          termsDesc = getdata[TERMSPOLICY][0][PAGE_CONTENT];
          isTermsShow = true;
        });
      }
    } else {
